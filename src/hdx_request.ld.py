...
def get_resources_url(tag, filters=[]):
	d = map(lambda x: map(lambda y: y['url'],x['resources']),hdx_request.show_tags(tag))
	if filters != []:
		urls = [y  for x in d for y in x if any([f in y for f in filters])]
	else :
		urls = [y  for x in d for y in x]
